...
/*
Our goal here is to take 1b and morph it to add object location context. The
       ...
       if (trim($_REQUEST['tb_ddWrap1']) == '' and 
        ...
        // process each vObj
           if (trim($_REQUEST['tb_ddWrap1'] != '')) {
               ...
               foreach( $rHsh as $k => $v) {
                 $s = preg_split("/##/", $k);
                 if (preg_match('/vDC1/', $s[1])){$_SESSION['vLC1'] .= $_SESSION['vDC1'];}  
                 if (preg_match('/vDC2/', $s[1])){$_SESSION['vLC1'] .= $_SESSION['vDC2'];} 
                 if (preg_match('/vDC3/', $s[1])){$_SESSION['vLC1'] .= $_SESSION['vDC3'];}                  
                 if (preg_match('/vDC4/', $s[1])){$_SESSION['vLC1'] .= $_SESSION['vDC4'];} 
                 if (preg_match('/vDC5/', $s[1])){$_SESSION['vLC1'] .= $_SESSION['vDC5'];}                 
               }
           }  else { $_SESSION['vLC1'] = "";}  	
           ...
           if (trim($_REQUEST['tb_ddWrap2'] != '')) {
               ...
               foreach( $rHsh as $k => $v) {
                 $s = preg_split("/##/", $k);
                 if (preg_match('/vDC1/', $s[1])){$_SESSION['vLC2'] .= $_SESSION['vDC1'];}  
                 if (preg_match('/vDC2/', $s[1])){$_SESSION['vLC2'] .= $_SESSION['vDC2'];} 
                 if (preg_match('/vDC3/', $s[1])){$_SESSION['vLC2'] .= $_SESSION['vDC3'];}                  
                 if (preg_match('/vDC4/', $s[1])){$_SESSION['vLC2'] .= $_SESSION['vDC4'];} 
                 if (preg_match('/vDC5/', $s[1])){$_SESSION['vLC2'] .= $_SESSION['vDC5'];}                
               }
           }  else { $_SESSION['vLC2'] = "";} 
           ...
           if (trim($_REQUEST['tb_ddWrap3'] != '')) {
               ...
               foreach( $rHsh as $k => $v) {
                 $s = preg_split("/##/", $k);
                 if (preg_match('/vDC1/', $s[1])){$_SESSION['vLC3'] .= $_SESSION['vDC1'];}  
                 if (preg_match('/vDC2/', $s[1])){$_SESSION['vLC3'] .= $_SESSION['vDC2'];} 
                 if (preg_match('/vDC3/', $s[1])){$_SESSION['vLC3'] .= $_SESSION['vDC3'];}                  
                 if (preg_match('/vDC4/', $s[1])){$_SESSION['vLC3'] .= $_SESSION['vDC4'];} 
                 if (preg_match('/vDC5/', $s[1])){$_SESSION['vLC3'] .= $_SESSION['vDC5'];}                 
               }
           }  else { $_SESSION['vLC3'] = "";} 
      }
     }	//end click event  
...
<div id="vObjConsole">
<h2>Event Console</h2>
<form name="form1" method="POST" action="jquery_events_drop_drag1c.php">
   <p>
